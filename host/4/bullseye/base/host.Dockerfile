...
RUN BUILD_NUMBER=$(echo ${HOST_VERSION} | cut -d'.' -f 3) && \
    ...
    dotnet publish -v q /p:BuildNumber=$BUILD_NUMBER /p:CommitHash=$HOST_COMMIT src/WebJobs.Script.WebHost/WebJobs.Script.WebHost.csproj -c Release --output /azure-functions-host --self-contained --runtime linux-x64 --self-contained && \
...
RUN apt-get update && \
    ...
    EXTENSION_BUNDLE_VERSION_V2=2.34.0 && \
    ...
    EXTENSION_BUNDLE_VERSION_V3=3.36.0 && \
    ...
    EXTENSION_BUNDLE_VERSION_V4=4.21.0 && \
